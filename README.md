...
| <a name="input_cloudfront_custom_headers"></a> [cloudfront\_custom\_headers](#input\_cloudfront\_custom\_headers) | Custom headers to add to the CloudFront response headers policy. By default Feature-Policy and X-Permitted-Cross-Domain-Policies are set | <pre>list(object({<br>  header   = string<br>  value    = string<br>  override = bool<br>}))</pre> | `[]` | no |
