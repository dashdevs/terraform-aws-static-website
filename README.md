...
| <a name="input_cloudfront_custom_headers"></a> [cloudfront\_custom\_headers](#input\_cloudfront\_custom\_headers) | Custom headers to add to the CloudFront response headers policy.  | `list(object({ header = string, value = string, override = bool }))` | `[]` | no |
